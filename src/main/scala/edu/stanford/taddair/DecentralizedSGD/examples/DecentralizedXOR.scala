...
import edu.stanford.taddair.DecentralizedSGD.actors.decentralized.DMaster
import edu.stanford.taddair.DecentralizedSGD.actors.decentralized.DMaster.{JobDone, Start}
...
class DecentralizedXOR extends Actor with ActorLogging {
  ...
  val DistBeliefMaster = context.actorOf(Props(new DMaster(
    dataSet = trainingSet,
    dataPerReplica = 2000,
