...
import edu.stanford.taddair.DecentralizedSGD.actors.centralized.CrossValidator.StartTraining
import edu.stanford.taddair.DecentralizedSGD.actors.centralized.Master.{JobDone, Start, ValidationDone}
...
class Master(dataSet: Seq[Example],
  ...
  for ((lr, i) <- learningRates.zipWithIndex) {
    crossValidatorActors(i) = context.actorOf(Props(new CrossValidator(
      modelId = i,
      dataSet = dataSet,
